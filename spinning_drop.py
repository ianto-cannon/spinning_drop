...
def spinning_drop_profile_solver(capillary_len, rad_tip, z_tip, lobes, fname=None):
  ds = 1e-4*z_tip
  psi=0
  r=0
  z=z_tip
...
for lobes in range(1,5): # 1,2,3,4..., dimensionless, number of bulges in z
  for r in range(10):
    ...
    for t in range(20):
      ...
      Volume, rad_neck, z, centroid, psi = spinning_drop_profile_solver(capillary_len, rad_tip, z_tip, lobes)
