...
def spinning_drop_profile_solver(capillary_len, rad_top, height, fname=None):
  rad_top/=capillary_len
  ...
  for i in range(int(5/ds)):
    ...
    r += dr
    z -= dz
    dPsi = ds * (2/rad_top + z**2/2/capillary_len**3 - np.sin(psi)/r)
    psi += dPsi
    Volume += np.pi*r**2*dz
...
rad_top = 5e-4 #m radius of curvature at the tip
