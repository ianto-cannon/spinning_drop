...
rotation_speed = 14.45 #rad/s, rotation speed of the drop
capillary_len = ( surf_tens / density / rotation_speed**2 ) ** (1/3)
nodes = 1 # 1,2,3,4..., dimensionless, number of times the profile is parallel to the z axis
z_tip = .89e-2 #m, distance of the tip from the axis of rotation
...
for t in range(50):
  ...
  Volume, rad_neck, z, centroid, psi = spinning_drop_profile_solver(capillary_len, rad_tip, z_tip, nodes)
  if psi < 0: rad_max = rad_tip
  elif psi > np.pi: rad_max = rad_tip
  elif z>0: rad_min = rad_tip
  else: rad_max = rad_tip
Volume, rad_neck, z, centroid, psi = spinning_drop_profile_solver(capillary_len, rad_min, z_tip, nodes, fname=f'data/spin.txt')
